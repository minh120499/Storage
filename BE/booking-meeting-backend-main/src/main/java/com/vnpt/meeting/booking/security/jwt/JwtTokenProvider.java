package com.vnpt.meeting.booking.security.jwt;
...
import com.vnpt.meeting.booking.config.RSAKeyProperties;
import com.vnpt.meeting.booking.constant.RedisKey;
import com.vnpt.meeting.booking.entity.UserDepartment;
import com.vnpt.meeting.booking.exception.InvalidTokenRequestException;
...
public class JwtTokenProvider {
    ...
    private Map<String, Object> buildClaims(UserDetailsImpl userDetails) {
        ...
        Set<UserDepartment> userDepartments = userDetails.getUserDepartments();
        if (userDepartments.size() > 0) {
            UserDepartment userDepartment = userDepartments.iterator().next();
            claims.put("departmentId", userDepartment.getDepartment().getId());
            claims.put("departmentName", userDepartment.getDepartment().getName());
        } else {
            claims.put("departmentId", 0);
