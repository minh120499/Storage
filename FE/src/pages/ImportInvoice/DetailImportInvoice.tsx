...
//   const [totalPriceReturnInvoice, setTotalPriceReturnInvoice] = useState(0);
  ...
  useEffect(() => {
    getDetailImportInvoice(code as string).then((result) => {
      ...
      getHistoryStatusImportInvoice(result.data?.anImport.id as number).then(
        (result) => {
  ...
  useEffect(() => {
    const invoiceStatusHistoryList = invoiceStatusHistory.filter(
      (obj: IHistoryStatus) => obj.statusName !== "Tạo phiếu trả hàng"
    );
          ...
          >
            ...
            <div style={{ width: "45%" }}>
              {(() => {
                const invoiceStatusHistoryList = invoiceStatusHistory.filter(
                  (obj: IHistoryStatus) =>
                    obj.statusName !== "Tạo phiếu trả hàng"
                );
                if (invoiceStatusHistoryList.length === 3) {
                  return (
