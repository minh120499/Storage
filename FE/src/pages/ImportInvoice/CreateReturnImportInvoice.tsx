...
import {
  IDetailImportInvoice,
  IImportReturnMyTableData,
...
const CreateReturnImportInvoice = () => {
  ...
  useEffect(() => {
    getImportReturn(code as string).then((result) => {
      const newResult = result.data.map((obj: IImportReturnMyTableData) => ({
        ...obj,
        inputQuantity: 0,
      }));
      setImportReturn(
        newResult.filter((obj: IImportReturnMyTableData) => obj.quantity > 0)
      );
    });
  ...
  const onSubmit = () => {
    ...
    importReturn.map((obj) => {
      if (obj.inputQuantity > 0) {
        list.push({
          detailsImportId: obj.detailsImportId as number,
          quantity: obj.inputQuantity as number,
        });
      }
      return null;
    });
    ...
    returnImportInvoice(
      returnImport,
      detailInvoices?.anImport.inventoryId as number
    ).then(() => {
